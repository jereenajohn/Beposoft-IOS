...
String formatCreatedAtDate(Map<String, dynamic> reportData) {
  final rawDate = reportData['created_at']; // Assuming it is a String like "2025-07-25T12:34:56Z"
...
// ...existing code...
  ...
  setState(() {
    if (query.isEmpty) {
      salesReportList = List.from(allSalesReportList); // Restore original data
    } else {
      salesReportList = allSalesReportList.where((product) {
        final bankName = product['bank_name']?.toString().toLowerCase() ?? '';
        final transactionId = product['transactionID']?.toString().toLowerCase() ?? '';
          ...
          children: [
            ...
            Expanded(
              child: RefreshIndicator(
                ...
                onPressed: () {
                  Navigator.push(context, MaterialPageRoute(builder: (context)=>UpdateBankTransferList(id: reportData['id'],))); 
